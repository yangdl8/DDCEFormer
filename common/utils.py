...
class AccumLoss(object):
    ...
    def update(self, val, n=1):
        ...
        if self.max<self.val: self.max=self.val
...
def save_model(args_checkpoint, args_previous_name, epoch, mpjpe, model, model_name):
    os.makedirs(args_checkpoint, exist_ok=True)

    if os.path.exists(args_previous_name):
        os.remove(args_previous_name)

    previous_name = '%s/%s_%d_%d.pth' % (args_checkpoint, model_name, epoch, mpjpe * 100)
